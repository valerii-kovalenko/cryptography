...
export class AES {
    ...
    private static coefMul(a: number[], b: number[], d: number[]) {
        d[0] = AES.gmul02(a[0], b[0]) ^ AES.gmul02(a[3], b[1]) ^ AES.gmul02(a[2], b[2]) ^ AES.gmul02(a[1], b[3]);
        d[1] = AES.gmul02(a[1], b[0]) ^ AES.gmul02(a[0], b[1]) ^ AES.gmul02(a[3], b[2]) ^ AES.gmul02(a[2], b[3]);
        d[2] = AES.gmul02(a[2], b[0]) ^ AES.gmul02(a[1], b[1]) ^ AES.gmul02(a[0], b[2]) ^ AES.gmul02(a[3], b[3]);
        d[3] = AES.gmul02(a[3], b[0]) ^ AES.gmul02(a[2], b[1]) ^ AES.gmul02(a[1], b[2]) ^ AES.gmul02(a[0], b[3]);
    }

    private static gmul02(a: number, b: number) {
        let p = 0;
        let highBit = 0;
    ...
    constructor(key: Uint8Array) {
        if (key.length !== 16) {
            throw new Error('Invalid key length(supports only 128 bites)');
        }
    ...
    private static roundConstant(i: number) {
        if (i == 1) {
            ...
            i--;
            while (i - 1 > 0) {
                AES.AES_R[0] = AES.gmul02(AES.AES_R[0], 0x02);
                i--;
            }
        }
...
export function test() {
    const str = 'textwithlength16';
    const key = new TextEncoder().encode(str);
    const message = 'Original text';

    const encoded = new AES(key).encString(message);

    log(`encoded for '${message}'`, encoded.toUpperCase());

    const aesDec = new AES(key).decString(encoded);

    log('decrypted', aesDec);
}
